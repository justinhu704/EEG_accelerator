...
// ==================================================
// Stage3 integration test through Conv3/BN3/ReLU3/MaxPool2 and RAM B.
// ==================================================

module tb_cnn_stage3;
    localparam int SAMPLES = 4;
    localparam int OUTPUT_SIZE = 18 * 1 * 15;
    ...
    initial begin
        ...
        require_file("../mem/weights/conv3_b.mem");
        require_file("../mem/weights/bn3_A.mem");
        require_file("../mem/weights/bn3_B.mem");
        $readmemh("../mem/golden/q_pool2_act.mem", expected_mem);
        ...
        // Pool2 results must be present at RAM B addresses 0 through 269.
        ...
        if ((stream_checked == OUTPUT_SIZE) &&
            ...
            $display("PASS: CNN Stage3 through Pool2 stored in RAM B.");
            $display("Pool2 stream checked=%0d max_diff=%0d",
                     stream_checked, stream_max_difference);
            $display("RAM B checked=%0d max_diff=%0d",
                     ram_checked, ram_max_difference);
        end else begin
            $display("FAIL: CNN Stage3 integration.");
    ...
    always @(posedge clk) begin
        ...
        if (output_valid) begin
            if ($isunknown(output_addr) || $isunknown(output_data) ||
                ...
                if (difference > TOLERANCE) begin
                    if (stream_errors < 20)
                        $display("Pool2 mismatch addr=%0d got=%0d expected=%0d diff=%0d",
        ...
        if (cycle_count > MAX_CYCLES) begin
            $display("FAIL: tb_cnn_stage3 timeout at cycle %0d", cycle_count);
            $finish;
        end
    end
endmodule
