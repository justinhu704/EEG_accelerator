...
module tb_conv1;
    ...
    .WEIGHT_FILE  ("../mem/weights/conv1_W.mem"),
    .BIAS_FILE    ("../mem/weights/conv1_b.mem")
) u_conv1 (
    .clk          (clk),
    .rst_n        (rst_n),
    .start        (start),
    ...
    initial begin
        ...
        $readmemh("../mem/golden/q_conv1_act.mem",
        ...
        if ((error_count == 0) && (checked_count == OUTPUT_SIZE)) begin
            $display("PASS: Conv1 matches MATLAB within 1 LSB.");
            ...
            $display("FAIL: Conv1 verification failed.");
            $display("Checked=%0d expected_count=%0d errors=%0d",
    ...
    always @(posedge clk) begin
        ...
        if (output_valid) begin
            if ($isunknown(output_addr) || $isunknown(output_data)) begin
                if (error_count < 20)
                    $display("MISMATCH: DUT produced X/Z at output %0d", checked_count);
                ...
                if (error_count < 20)
                    $display("MISMATCH: golden value is X/Z at addr=%0d", output_addr);
                error_count = error_count + 1;
