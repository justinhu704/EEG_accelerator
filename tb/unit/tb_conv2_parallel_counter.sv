...
// Compares the original formula-address parallel Conv2 against the new
// incremental-counter implementation. Different internal address generation
// is allowed, but all 57,760 addressed outputs must match exactly.
module tb_conv2_parallel_counter;
    ...
    localparam int WEIGHT_DEPTH = K_H * K_W * IN_CH * OUT_GROUPS;
    localparam int BIAS_DEPTH = OUT_GROUPS;
    ...
    logic [31:0] new_input_addr, new_weight_addr, new_bias_addr;
    logic [31:0] new_output_addr, new_output_ch;
    logic signed [15:0] new_input_data, new_output_data;
    logic signed [(16*LANES)-1:0] new_weight_data, new_bias_data;
    ...
    integer old_done_cycle, new_done_cycle;
    ...
    weight_rom #(
        .DATA_W(16*LANES), .DEPTH(WEIGHT_DEPTH),
        .ADDR_W($clog2(WEIGHT_DEPTH)),
        .MEM_FILE("../mem/weights/conv2_W_x4.mem")
    ) u_old_weight_rom (
        .clk(clk),
        .addr(old_weight_addr[$clog2(WEIGHT_DEPTH)-1:0]),
    ...
    weight_rom #(
        ...
        .clk(clk),
        .addr(new_weight_addr[$clog2(WEIGHT_DEPTH)-1:0]),
        .data(new_weight_data)
    );

    weight_rom #(
        .DATA_W(16*LANES), .DEPTH(BIAS_DEPTH),
        .ADDR_W($clog2(BIAS_DEPTH)),
        .MEM_FILE("../mem/weights/conv2_b_x4.mem")
    ) u_old_bias_rom (
        .clk(clk), .addr(old_bias_addr[$clog2(BIAS_DEPTH)-1:0]),
        .data(old_bias_data)
    );
    ...
    weight_rom #(
        ...
        .clk(clk), .addr(new_bias_addr[$clog2(BIAS_DEPTH)-1:0]),
        .data(new_bias_data)
    );
    ...
    initial begin
        ...
        require_file("../mem/weights/conv2_W_x4.mem");
        require_file("../mem/weights/conv2_b_x4.mem");
        ...
        for (compare_index = 0; compare_index < OUTPUT_SIZE;
            ...
            if (!old_seen[compare_index] || !new_seen[compare_index] ||
                (old_results[compare_index] !==
                 new_results[compare_index])) begin
                if (error_count < 20)
        ...
        if (error_count == 0) begin
            $display("PASS: counter-address Conv2 exactly matches original.");
            $display("Checked=%0d old_cycles=%0d new_cycles=%0d",
                     OUTPUT_SIZE, old_done_cycle, new_done_cycle);
        end else begin
            $display("FAIL: counter-address Conv2 errors=%0d", error_count);
        end
        $finish;
    end
    ...
    always @(posedge clk) begin
        ...
        if (old_valid) begin
            if ($isunknown(old_output_addr) ||
                (old_output_addr >= OUTPUT_SIZE) ||
                old_seen[old_output_addr]) begin
                error_count = error_count + 1;
            end else begin
        ...
        if (new_valid) begin
            if ($isunknown(new_output_addr) ||
                (new_output_addr >= OUTPUT_SIZE) ||
                new_seen[new_output_addr]) begin
                error_count = error_count + 1;
            end else begin
        ...
        if (cycle_count > MAX_CYCLES)
            $fatal(1, "Conv2 counter comparison timeout at cycle %0d",
                   cycle_count);
    end
endmodule
